...
public class JobPostingController {
    ...
    @PutMapping("/{id}")
    @Operation(summary = "채용 공고 수정 (STAFF 이상)",description="기업의 채용 공고를 수정합니다.", security = { @SecurityRequirement(name = "Authorization") })
    @ApiResponses(value = {
    ...
    @DeleteMapping({"/{id}"})
    @Operation(summary = "채용 공고 삭제 (STAFF 이상)",description="기업의 채용 공고를 삭제합니다.", security = { @SecurityRequirement(name = "Authorization") })
    @ApiResponses(value = {
            @ApiResponse(
                    responseCode = "200",
                    description = "삭제 성공",
    ...
    @Operation(summary = "공고의 일정 추가 (STAFF 이상)",description="해당 공고의 일정을 추가합니다.", security = { @SecurityRequirement(name = "Authorization") })
    @ApiResponses(value = {
